...
const { data: spell } = await useAsyncData(`fetchSpellsById-${spellId}`, async () => {
...
<template>
  <div v-if="spell">
    ...
    <p v-if="spell.light === 'Transparent' || spell.light === 'icyBlue'" :style="{ color: 'black' }">
     Цвет заклинания: {{ spell.light }}
    </p>
    <p v-else :style="{color: spell.light}">Цвет заклинания: {{ spell.light}}</p>
